...
{
	public class ExpectToReplyAction<TMessage, TStateMachine> :
		...
		where TMessage : IMessage
	{
		...
		public ExpectToReplyAction(IContainer container,
		...
		public override Action<IMessage> CreateExpectation()
		{
			...
			replyAction = (sm) => CreateReplyExpectation(message, sm);
		...
		{
			string verifiable = string.Format("The state machine '{0}' should reply with the message '{1}' " +
			...
			if (specification != null)
				specification.VerifyReply(message, verifiable);
		}
	}
}