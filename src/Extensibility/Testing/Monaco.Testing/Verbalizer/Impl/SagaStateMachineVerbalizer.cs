...
namespace Monaco.Testing.Verbalizer.Impl
{
	/// <summary>
	...
	public class SagaStateMachineVerbalizer : ISagaStateMachineVerbalizer
	{
		...
		public string Verbalize(Type stateMachine)
		{
			string results = string.Empty;

			if(typeof(SagaStateMachine).IsAssignableFrom(stateMachine))
				throw new InvalidOperationException(string.Format("The type '{0}' must be derivable from '{1}'", 
					stateMachine.FullName, typeof(SagaStateMachine<>)));

			SagaStateMachine machine = stateMachine.Assembly
				.CreateInstance(stateMachine.FullName) as SagaStateMachine;

			if(machine != null)
			{
				results = this.Verbalize(machine);
			}
			...
			where TSagaStateMachine : SagaStateMachine
		{
			...
			var bus = MockFactory.CreateServiceBusMock(Configuration.Configuration.Instance.Container);
