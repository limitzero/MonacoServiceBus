...
{
	public class StartMessageModulesFilter : IPipelineFilter
	{
		private readonly IContainer container;
	
		public StartMessageModulesFilter(IContainer container)
		{
		...
		public void Execute(IEnvelope envelope)
		{
		    ...
		    foreach (var message in envelope.Body.Payload)
			{
				try
				{
					envelope.Header.RecordStage(module, message, "Start Message Module - " + module.GetType().Name);
					module.OnMessageStartProcessing(this.container, message);
				}
				catch (Exception startModuleException)
				{
					string msg =
						string.Format(
							"An error has occurred while attempting to execute the message module '{0}' for the start processing action, " +
