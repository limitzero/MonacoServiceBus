...
namespace Monaco.Testing.StateMachines.Impl
{
		...
		where TStateMachine : SagaStateMachine
	{
		...
		public StateMachineTestScenario(
			...
			: this(consumedMessage, null, consumeMessageAction, stateMachine, kernel, mockServiceBus)
		{
		}
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectToPublish<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectToPublishAction<T, TStateMachine>(Kernel,
			                                                                      StateMachine,
			                                                                      MockServiceBus,
			                                                                      ConsumedMessage,
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectNotToPublish<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectNotToPublishAction<T, TStateMachine>(Kernel,
			                                                                         StateMachine,
			                                                                         MockServiceBus,
			                                                                         ConsumedMessage,
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectToSend<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectToSendAction<T, TStateMachine>(Kernel,
			                                                                   StateMachine,
			                                                                   MockServiceBus,
			                                                                   ConsumedMessage,
			                                                                   messageConstructionAction).
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectNotToSend<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectNotToSendAction<T, TStateMachine>(Kernel,
			                                                                      StateMachine,
			                                                                      MockServiceBus,
			                                                                      ConsumedMessage,
			                                                                      messageConstructionAction)
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectReplyWith<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectToReplyAction<T, TStateMachine>(Kernel,
			                                                                    StateMachine,
			                                                                    MockServiceBus,
			                                                                    ConsumedMessage,
			                                                                    messageConstructionAction)
		...
		public IStateMachineTestScenario<TStateMachine> ExpectNotToReplyWith<T>(Action<T> messageConstructionAction)
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectNotToReplyAction<T, TStateMachine>(Kernel,
			                                                                       StateMachine,
			                                                                       MockServiceBus,
			                                                                       ConsumedMessage,
			                                                                       messageConstructionAction)
		...
		public virtual IStateMachineTestScenario<TStateMachine> ExpectToRequestTimeout<T>(TimeSpan delayDuration,
			...
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectToRequestTimeoutAction<T, TStateMachine>(Kernel,
			                                                                             StateMachine,
			                                                                             MockServiceBus,
			                                                                             delayDuration,
			                                                                             ConsumedMessage,
		...
		public IStateMachineTestScenario<TStateMachine> ExpectNotToDelay<T>(TimeSpan delayDuration,
			...
			where T : IMessage
		{
			Action<IMessage> verify = new ExpectNotToDelayAction<T, TStateMachine>(Kernel,
			                                                                       StateMachine,
			                                                                       MockServiceBus,
			                                                                       delayDuration,
			                                                                       ConsumedMessage,
		...
		private void CheckForExternallyConfigurationForConsume()
		{
			...
			else
			{
				...
				ConsumeMessageAction = () =>
				                       	{
				                       		var dispatcher = Kernel.Resolve<ISagaStateMachineMessageDispatcher>();
